<template>
  <div class="main flex-col flex-x-center flex-y-center">
    <div class="center">
      <!--头部-->
      <my-head/>
      <!--轮播图，最新资讯-->
      <!--轮播图-->
      <div class="news flex-row flex-space-between">
        <div class="news-left">
          <el-carousel height="257px">
            <el-carousel-item>
              <img src="../../static/img/photo.png" alt="">
            </el-carousel-item>
            <el-carousel-item>
              <img src="../../static/img/photo1.jpg" alt="">
            </el-carousel-item>
            <el-carousel-item>
              <img src="../../static/img/photo2.jpg" alt="">
            </el-carousel-item>
            <el-carousel-item>
              <img src="../../static/img/photo3.jpg" alt="">
            </el-carousel-item>
          </el-carousel>
        </div>
        <!--最新资讯-->
        <div class="flex-col flex-left ">
          <img src="../../static/img/news-title.png" alt="">
          <div class="news-right flex-row flex-space-between">
            <img src="../../static/img/triangle.png" alt="">
            <div class="news-right-inner flex-col flex-space-between" >
              <span v-for="item in items1">
                <router-link :to="{path:'/Inside'}" class="flex-row flex-space-between" >
                <p>{{item.message}}</p>
                <p>2017-07-10</p>
              
              </router-link>
              </span>
          
            </div>
          </div>
        </div>

      </div>
      <!--学校公告，招聘招生-->
      <div class="show flex-row flex-left flex-space-between">
        <div class="flex-col flex-left">
          <img src="../../static/img/notice.png" alt="">
          <div class="show-notice flex-row flex-space-between">
            <img src="../../static/img/triangle.png" alt="">
            <div class="show-notice-inner flex-col flex-space-between"  >
              <span v-for="item in items2">
                <router-link :to="{path:'/Inside'}" class="flex-row flex-space-between" >
                <p>{{item.message}}</p>
                <p>2017-07-10</p>
              
              </router-link>
              </span>
            </div>
          </div>
        </div>
        <!--招聘招生-->
        <div class="flex-col flex-left flex-space-between">
          <img src="../../static/img/invite.png" alt="">
          <div class="show-invite flex-row flex-space-between">
            <img src="../../static/img/triangle.png" alt="">

            <div class="show-invite-inner flex-col flex-space-between" >
              <span v-for="item in items3">
                <router-link :to="{path:'/Inside'}" class="flex-row flex-space-between" >
                <p>{{item.message}}</p>
                <p>2017-07-10</p>
              </router-link>
              </span>
              
            </div>
          </div>
        </div>
      </div>
      <!--底部-->
      <my-bottom/>
    </div>
  </div>
</template>

<script>
  export default {
    data(){
      return{
        items1:[
          {message:'2017年中考报喜'},
          {message:'2017年高考报喜'},
           {message:'2016届广州市越秀区初中毕业...'},
          {message:'化学科喜讯'},
           {message:'化学科喜报'},
          {message:'体育科喜讯'},
          {message:'体育科喜讯'}
        ],
        items2:[
          {message:'2017高一分班公布'},
          {message:'高一新生取得住宿资格名单'},
           {message:'2017高一新生住宿特别通知'},
          {message:'培“培英计划”广州培道遂志班通知道校训'},
           {message:'2017培道高一新生住宿申请'},
          {message:'2017高一新生报到分组名单'},
          {message:'遂志班招生简章'}
        ],
        items3:[
          {message:'校长简介'},
          {message:'东山钟灵，培道毓秀'},
           {message:'A Brief Introduction To...'},
          {message:'培道校训'},
           {message:'广州培道教学理念————个性、...'},
          {message:'校园歌曲《培道艳》'},
          {message:'2017年越秀区初中体育特长生招生方案'},
        ]
      }
    }
  }
</script>
