    ...
    <div class="center">
      ...
      <div class="show flex-row flex-left flex-space-between">
        ...
        <el-carousel :interval="4000" type="card" height="200px">
          <el-carousel-item v-for="item in src" :key="item">
          <el-image :src=item></el-image>
          <router-link :to="{path:'/Notice'}" class="flex-row flex-space-between" >
          el-image :src=item></router-link>
...
export default {
  data() {
    return {
      ...
      items2: [
        { message: '2017高一分班公布' },
        { message: '高一新生取得住宿资格名单' },
        { message: '2017高一新生住宿特别通知' },
        { message: '培“培英计划”广州培道遂志班通知道校训' },
        { message: '2017培道高一新生住宿申请' },
        { message: '2017高一新生报到分组名单' },
        { message: '遂志班招生简章' }
      ],
      items3: [
        { message: '校长简介' },
        { message: '东山钟灵，培道毓秀' },
        { message: 'A Brief Introduction To...' },
        { message: '培道校训' },
        { message: '广州培道教学理念————个性、...' },
        { message: '校园歌曲《培道艳》' },
        { message: '2017年越秀区初中体育特长生招生方案' },
      ]
    }
  },
  mounted() {
  ...
  methods: {
    ...
    getimg() {
      // let url = 'http://127.0.0.1:8080/news/all'

      // this.$axios.get(url).then(res => {
      //   console.log(res.data)
      //   for (let i = 0; i < res.data.length; i++) {
      //     let course = res.data[i].img;
      //     console.log(course);
      //     this.src.push(course)

      //   }
      // }
      this.src.push('http://localhost:8080/image/news/5.jpg'),
        this.src.push('http://localhost:8080/image/news/6.jpg'),
        this.src.push('http://localhost:8080/image/news/7.jpg'),
        this.src.push('http://localhost:8080/image/news/8.jpg'),
