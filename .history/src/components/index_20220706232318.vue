      ...
      <div class="show flex-row flex-left flex-space-between">
        <div class="flex-col flex-left">
          <el-card class="box-card">
            ...
            <el-row>
              <el-col :span="11" v-for="(item, index) in items2" :key="item" :offset="index > 0 ? 2 : 0">
                <el-card :body-style="{ padding: '0px' }">
                  ...
                  <div style="padding: 14px;">
                      ...
                      <router-link :to="{path:'/News'}" class="flex-row flex-space-between" >{{item.message}}</router-link>
    ...
    getnews() {
      ...
      this.$axios.get(url).then(res => {
        ...
        for (let i = 0; i < res.data.length; i++) {
          ...
          imgnews['message'] = newstitle;
          imgnews['image'] = newsimage;
          // console.log(imgnews)
          if (i < 2){
            this.items2.push(imgnews);
            console.log(this.items2)
          }
          this.src1.push(newsimage)
          if (i <= 7) {
            this.items3.push(newstitle)
          }
