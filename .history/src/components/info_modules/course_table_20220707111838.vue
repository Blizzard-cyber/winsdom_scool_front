    ...
    <div class="block">
        <el-table
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <div>
                 <a @click="drawer=true" style="color:blue;cursor:pointer">{{scope.row.textbook.name}}</a>
      ...
      this.$axios.get('http://127.0.0.1:8080/courseTable/' + cookie_id + '/' + this.semester).then(res => {
        for (let i = 0; i < res.data.length; i++){
          ...
          if (res.data[i].textbook) {
            course['textbook']['name']=res.data[i].textbook.name;
            course['textbook']['id'] = res.data[i].textbook.id;
