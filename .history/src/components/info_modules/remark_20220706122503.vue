    ...
    <div class="center">
      ...
      <el-page-header @back="goBack" content="我的成绩"></el-page-header>
        <div style="margin-top: 20px;display: flex;justify-content: center">
            <el-input v-model="keywords" placeholder="通过课程名称搜索..." prefix-icon="el-icon-search"
            ...
            <el-card class="hr-card" v-for="(user,index) in users" :key="index">
                ...
                <div>
                    <div class="img-container">
                        <img :src="@/../../static/img/logolo.png" :alt="user.name" :title="user.name" class="userface-img">
                    </div>
                    <div class="userinfo-container">
                        <div>课程代码：{{user.id}}</div>
                        <div>考试科目：{{user.name}}</div>
                        <div>任课老师：{{user.teacher}}</div>
                        
                        <div>成绩：{{user.score}}</div>
                        <div>备注：{{user.description}}</div>
                        <!-- <div>用户角色：
